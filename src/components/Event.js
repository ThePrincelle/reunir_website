		...
		>
			<div className="flex-shrink-0 object-cover max-h-18 h-18">
				...
				>
					...
					/>
					<Marker position={[props.event.lieu.lat, props.event.lieu.lng]}>
						<Popup>
							<a href={`https://maps.google.com/?q=${props.event.lieu.lat},${props.event.lieu.lng}`} target="_blank">Google Maps</a>
						</Popup>
					</Marker>
				</MapContainer>
			</div>
			<div className="flex-1 bg-white p-6 flex flex-col justify-between">
				<div className="flex-1 pb-5">
					...
					<a href={`https://maps.google.com/?q=${props.event.lieu.lat},${props.event.lieu.lng}`} className="text-sm font-medium text-justify pt-2 leading-3 text-green-600">
						{props.event.lieu.address.split(', ').slice(0, -2).join(', ')}
					</a>
					<div
				...
				<div className="mt-4 flex items-center">
					<div className="flex-shrink-0">
						<span className="sr-only">{new Date(props.event.date).toLocaleDateString('fr-FR', dateOptions)}</span>
					</div>
					<div className="">
						<p className="text-sm font-medium text-gray-900">
							{new Date(props.event.date).toLocaleDateString('fr-FR', dateOptions)}
						</p>
